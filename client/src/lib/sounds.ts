...
export const SOUND_META: Record<SoundKey, { label: string; description: string }> = {
  ...
  blackjack: { label: "Blackjack!",      description: "Plays when your hand is a natural 21" },
  bust:      { label: "Bust",            description: "Plays when your hand goes over 21" },
  push:      { label: "Push",            description: "Plays when your hand ties with the dealer" },
};
...
export const SOUNDS_BY_GAME_TYPE: Record<string, SoundKey[]> = {
  ...
  "blackjack":              ["win", "lose", "victory", "defeat", "blackjack", "bust", "push"],
};

// Default URLs. Partial because some keys (e.g. push) have no built-in default —
// they stay silent until the deck owner sets a custom sound via SoundPicker.
const URLS: Partial<Record<SoundKey, string>> = {
  ...
  // Defaults recycled from the existing library so the slots aren't silent
  // out of the box. Users can override any of them.
  blackjack: `${BASE}/final-fantasy-vii-victory-fanfare-1.mp3`,
  bust:      `${BASE}/sad-trombone-sound-effect-wah-wah-wah-fail-sound-fail-horns.mp3`,
};
...
export function preloadSounds() {
  ...
  Object.values(URLS).forEach((url) => { if (url) new Audio(url); });
}
